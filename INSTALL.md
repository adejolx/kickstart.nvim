...
- [`ripgrep`](https://github.com/BurntSushi/ripgrep) for project search and file discovery
- [`fd`](https://github.com/sharkdp/fd) is optional; Telescope can use it as an alternative file finder
...
If plugins do not install, check that `git` is available and run `:checkhealth`. If Telescope's native FZF extension is missing, install `make` and restart Neovim. If search mappings return no results, confirm that `rg` is on your `PATH`; `fd` is optional.
